...
import { getSalesOrder, createReservation } from '../api/salesApi.js'
...
export function ReservationFormPage() {
    ...
    async function loadOrder() {
      try {
        ...
        if (!validStatuses.includes(loaded.status)) {
          throw new Error(`This order (${loaded.status.replace(/_/g, ' ')}) is not in a reservable state. Confirm the order first.`)
        }

        setOrder(loaded)

        // Pre-fill items with pending reservation quantities
        const defaultItems = (loaded.items || [])
          .map(item => {
            const pending = Math.max(0, item.orderedQuantity - item.reservedQuantity)
            return {
              ...
              alreadyReserved: item.reservedQuantity,
              pendingQuantity: pending,
              reservedQuantity: pending.toString(),
            }
          })
  ...
  function handleItemChange(idx, value) {
    form.setValues(c => {
      const items = [...c.items]
      items[idx] = { ...items[idx], reservedQuantity: value }
      return { ...c, items }
    })
  }
  ...
  async function handleSubmit(e) {
    ...
    const payloadItems = form.values.items.filter(i => Number(i.reservedQuantity) > 0)
    ...
    try {
      ...
      const payload = {
        reservationDate: form.values.reservationDate,
      ...
      const res = await createReservation(orderId, payload)
      navigate(`/app/sales/reservations/${res.data?.id || ''}`, { replace: true })
    } catch (error) {
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-[#E5E7EB] bg-white p-6">
        <div className="mb-6 border-b border-[#E5E7EB] pb-5">
          <h2 className="text-lg font-semibold text-[#111827]">Reserve Stock</h2>
          <p className="mt-1 text-sm text-[#6B7280]">
            Create a stock reservation against order:{' '}
        ...
        {order && (
          <form onSubmit={handleSubmit} className="grid gap-6">
            <div className="grid gap-4 md:grid-cols-2">
              <FormField
                label="Reservation Date" type="date" name="reservationDate"
                value={form.values.reservationDate} onChange={handleChange}
                error={form.errors.reservationDate} required
              />
              <FormTextarea
            ...
            <div className="rounded-lg border border-[#E5E7EB] bg-[#F9FAFB] p-4">
              <p className="mb-4 text-sm font-semibold text-[#111827]">Items to Reserve</p>

              {form.values.items.length === 0 ? (
                <p className="py-4 text-center text-sm font-semibold text-[#22C55E]">
                  All items on this order are already fully reserved.
                </p>
              ) : (
                      ...
                      <div className="flex flex-col gap-4 md:flex-row md:items-center">
                        <div className="flex-1">
                          ...
                          <p className="mt-1 text-xs text-[#6B7280]">
                            ...
                            Already reserved: <span className="font-semibold text-[#111827]">{item.alreadyReserved}</span> &nbsp;Â·&nbsp;
                        ...
                        <div className="w-44">
                          <FormField
                            ...
                            error={form.errors[`items.${idx}.reservedQuantity`]}
                            required
                          />
                        </div>
                      </div>
            ...
            {canEdit && (
              <div className="flex flex-wrap gap-3">
                ...
                >
                  {form.isSubmitting ? 'Creating...' : 'Create Reservation'}
                </button>
                <Link
